...
function backupOriginalFile() {
    if (fs.existsSync(sourceJsonPath)) {
        ...
        console.warn(`Source file not found at: ${sourceJsonPath}`);
        console.log(`Looking for JSON file at: ${sourceJsonPath}`);
        // Check if parent directories exist
        console.log(`Parent directory exists: ${fs.existsSync(path.dirname(sourceJsonPath))}`);
    }
}
...
async function convertToPercentages() {
    try {
        ...
        if (!fs.existsSync(sourceJsonPath)) {
            throw new Error(`Source file not found: ${sourceJsonPath}`);
        }
        ...
        for (const [filename, points] of Object.entries(originalPoints)) {
            ...
            if (!fs.existsSync(imagePath)) {
                console.warn(`Skipping ${filename}: Image file not found at ${imagePath}`);
                skipped++;
                continue;
            }
...
function applyConversion() {
    try {
        ...
        console.error(`Error applying conversion: ${err.message}`);
    }
}
